...
#        prop_surface.SetOpacity(0.5)
        ...
        if input == 'y':
            ...
            if ID == None:
                ...
                if ID=='GroupIds':
                    ...
                    mapper.SetprodottoscalareRange(self.Surface_bc.GetPointData().GetScalars().GetRange(0))
                else:
                    self.Surface_bc.GetCellData().SetActiveScalars(ID)            
                    mapper.SetprodottoscalareModeToUseCellData()
                mapper.SetprodottoscalareRange(self.Surface_bc.GetPointData().GetScalars().GetRange(0))
                actor = vtk.vtkActor()
                actor.SetMapper(mapper)
                prodottoscalareBarActor = vtk.vtkprodottoscalareBarActor()
                prodottoscalareBarActor.SetLookupTable(actor.GetMapper().GetLookupTable())
                prodottoscalareBarActor.GetLabelTextProperty().ItalicOff()
                prodottoscalareBarActor.GetLabelTextProperty().BoldOff()
                prodottoscalareBarActor.GetLabelTextProperty().ShadowOff()
                prodottoscalareBarActor.SetLabelFormat('%1.0f')
                prodottoscalareBarActor.SetTitle(ID)
                self.actor.append(actor) #posizione 0
                self.actor.append(prodottoscalareBarActor) #posizione 1
                prop = []
                tmp=self.Surface_bc.GetPointData().GetScalars().GetRange(0)
    ...
    def ViewerBC(self, ID='GroupIds'):#visualizza la superficie colorata secondo ID: CenterlineIds, GroupIds, TractIds, Blanking
        ...
        prodottoscalareBarActor.GetLabelTextProperty().ShadowOff()
        prodottoscalareBarActor.SetLabelFormat('%1.0f')
        prodottoscalareBarActor.SetTitle('GroupIds')
        
        renderer = vtk.vtkRenderer()
        renderer.AddActor(actor)
        renderer.AddActor(prodottoscalareBarActor)
...
#                            print 'BC:'+str(i)+' Sections_resampled:'+str(j), self.Branches[i].Sections_resampled[j]
                            if self.Branches[i].Sections_resampled[j] != None:
                                ac+=self.Branches[i].Sections_resampled[j].createActor()
...
#                    ac.append(vtk_methods.CreateArrow(tempCL.Centerline, [1, 0, 0]))
    ...
    def ReorderPoints(self, reorder = [None, None]):
        """ Riordina i punti
        reorder: [0] - metodologia di ricampionamento ( parallel, boundaries,parallel+boundaries = 'new') [1] - (solo per boundaries, id del punto p)
        """
        ...
        elif reorder[0] == 'new' and reorder[1] != None:
            #prima riordino con first
            self.ReorderPoints(['first', None])
            #ora calcolo il nuovo id
            for i in self.BifGroupId:
                # Branch IN
                self.Branches[ self.Branches[i].adjacentIDs[0][0]].ReorderPoint([reorder[0], self.Branches[i].p[reorder[1] ]], 'in')
                # Branch OUT
    ...
    def ReorderPoints2(self,  reorder = 4):
        print 'Riordinamento dei punti'
        #prima riordino con first
        self.ReorderPoints(['first', None])
        #seleziono le biforcazioni
        
        for i in self.BifGroupId:
            print 'Biforcazione id:', i
            # Branch IN
            self.Branches[ self.Branches[i].adjacentIDs[0][0]].ReorderPoint2(self.Branches[i].p[reorder], 'in')
            # Branch OUT
            for k in range(2):
                self.Branches[ self.Branches[i].adjacentIDs[1][k]].ReorderPoint2(self.Branches[i].p[reorder], 'out')
    ...
    def computeBoundariesPoints(self, input=50):
        ...
        for i in ids:
            ...
            n.append(vtk_methods.versors(vtk_methods.prodottovettoriale(n[1], n[2])))
            n.append(vtk_methods.versors(vtk_methods.prodottovettoriale(n[2], n[1])))
            ...
            for k in range(6): #scorro tutti i p
                ...
                for m in range(len(boundaries)): # scorro le boundaries
                    ...
                    p[m].append(0)
                    zero = math.acos(vtk_methods.prodottoscalare( n[k+3],  vtk_methods.normalize(vtk_methods.vettore(O, boundaries[m][0]))))
                    p[m][k]=0
                    for j in range(len(boundaries[m])): #scorro i punti
                        a = math.acos(vtk_methods.prodottoscalare( n[k+3],  vtk_methods.normalize(vtk_methods.vettore(O, boundaries[m][j]))))
                        if a < zero:
                            p[m][k] = j
                            zero = a
    ...
    def CreateSectionsVTP(self):
        ...
        for i in range(len(self.Branches)):
            if self.Branches[i].Blanking == False:                
                for j in range(len(self.Branches[i].Sections_resampled)):
                    if self.Branches[i].Sections_resampled[j] != None:
                        print i, j, self.Branches[i].Sections_resampled[j].selectSurface().GetNumberOfPoints()
                        
                        if self.Branches[i].Sections_resampled[j].selectSurface().GetNumberOfPoints() > 0:
                            appendFilter.AddInput(self.Branches[i].Sections_resampled[j].selectSurface())
        appendFilter.Update()
        cleanFilter = vtk.vtkCleanPolyData()
        try:
            cleanFilter.SetInputConnection(appendFilter.GetOutputPort()) #se non funziona usare SetInput e GetOutput\
        except:
            cleanFilter.SetInput(appendFilter.GetOutput())
        cleanFilter.Update()
        printings.append(self.dir+self.fn_base+'_all_sections.vtp')
        print cleanFilter.GetOutput()
        vtk_methods.WritePolyData(cleanFilter.GetOutput(), self.dir+self.fn_base+'_all_sections.vtp')
#        self.Viewer(cleanFilter.GetOutput())
...
#            print myPype.GetScriptObject('vmtkcenterlineattributes', '0').Centerlines.GetPointData().GetArray('Abscissas')
    ...
    def Viewer_ID(self, ID='GroupIds'): #visualizza la centerline colorata secondo ID: CenterlineIds, GroupIds, TractIds, Blanking
        ...
        labelsMapper.SetLabelModeToLabelprodottoscalares()        
        ...
        centerlineMapper.prodottoscalareVisibilityOn()
        centerlineMapper.SetprodottoscalareModeToUseCellData()
        self.Centerlinetmp[1].GetCellData().SetActiveScalars(ID)
        centerlineMapper.SetprodottoscalareRange(self.Centerlinetmp[1].GetCellData().GetScalars().GetRange(0))
        
        centerlineActor = vtk.vtkActor()
        centerlineActor.SetMapper(centerlineMapper)
        
        prodottoscalareBarActor = vtk.vtkprodottoscalareBarActor()
        prodottoscalareBarActor.SetLookupTable(centerlineActor.GetMapper().GetLookupTable())
        prodottoscalareBarActor.GetLabelTextProperty().ItalicOff()
        prodottoscalareBarActor.GetLabelTextProperty().BoldOff()
        prodottoscalareBarActor.GetLabelTextProperty().ShadowOff()
        ##             self.prodottoscalareBarActor.GetLabelTextProperty().SetColor(0.0,0.0,0.0)
        prodottoscalareBarActor.SetLabelFormat('%1.0f')
        prodottoscalareBarActor.SetTitle('GroupIds')
...
#                print i, j
    ...
    def findFirstBoundariesDistance(self, point = None, last=None):
...
#                            print ids[k], pto
    ...
    def findFirstBoundariesAngle(self, p=None, out=None):
        """ Calcola l'id del nuovo punto first delle sezioni andando a trovare il punto che ha angolo minore tra vettore (punto centrale della boundaries,punto1)
        e il vettore(punto centrale sezione, punto intersezione piano sezione e retta parallela alla congiungente dei punti centrali sezione-boundaries passante per il punto 1 delle boundaries
        """
        debug =  0
        if out == 'in':
            # 1) prendo i punti necessari al calcolo

            p0s = self.Sections_resampled[self.selectSection('last')].clPoint  #punto centrale sezione
            p0b = self.midPointBoundaries[1] #punto centrale boundaries
            p1s = self.Sections_resampled[self.selectSection('last')].selectSurface().GetPoints().GetPoint(0) #first sezione
            p1b = self.Sections_resampled[len(self.Sections_resampled)-1].selectSurface().GetPoints().GetPoint(0) #first boundaries
            i1 = vtk_methods.vettore(p0s, p1s)
            k1 = self.Sections_resampled[self.selectSection('last')].Normal
            j1= vtk_methods.prodottovettoriale(i1, k1) #n x v
            # calcolo la proiez ortog sul piano ||P,n||
            vec1 = vtk_methods.proiezionePuntosuPiano(p0s, k1, p1b)
            #ora cerco l'id del nuovo first          
            vec1versore=vtk_methods.normalize(vtk_methods.vettore(p0s, vec1))
            id, point = self.Sections_resampled[self.selectSection('last')].findFirstAngle(vtk_methods.vettore(p0s, vec1))
              
#            id = self.Sections_resampled[self.selectSection('last')].findFirstAngle(vtk_methods.vettore(p0s, vec1))
            if debug == 1:
                ac=[]
                points = self.Sections_resampled[self.selectSection('last')].selectSurface().GetPoints()
                for i in range(0, points.GetNumberOfPoints()):
                    if i== 0:
                        ac.append(vtk_methods.CreateSphere(points.GetPoint(i), 0.1, [1, 0, 1]))
                    else:
                        ac.append(vtk_methods.CreateSphere(points.GetPoint(i), 0.1, [0, 0, 1]))
                    print i, math.acos(vtk_methods.prodottoscalare(vec1versore, vtk_methods.normalize(vtk_methods.vettore(p0s, points.GetPoint(i)))))
                    
                ac += vtk_methods.CreateVersor(p0s, vtk_methods.vettore(p0s, vec1), colors[6])
                ac.append(vtk_methods.CreateSphere(vec1, 0.11, colors[6]))
                self.viewActors(ac)
                ac=[]
                ac += vtk_methods.CreateVersor(p0s, i1,  colors[1])
                ac += vtk_methods.CreateVersor(p0s, j1, colors[2])
                ac += vtk_methods.CreateVersor(p0s, k1, colors[3])
                ac += vtk_methods.CreateVersor(p0s, vtk_methods.vettore(p0s, p1b), colors[4])
                ac += vtk_methods.CreateVersor(p0s, vtk_methods.vettore(p0s, vec1), colors[6])
                ac.append(vtk_methods.CreateSphere(p1b, 0.31, colors[4]))
                ac.append(vtk_methods.CreateSphere(vec1, 0.31, colors[6]))          
                ac+=self.Sections_resampled[self.selectSection('last')].createActor()
            ...
            debug = 1
            # 1) prendo i punti necessari al calcolo
            p0s = self.Sections_resampled[1].clPoint  #punto centrale sezione
            p0b = self.midPointBoundaries[0] #punto centrale boundaries
            p1s = self.Sections_resampled[1].selectSurface().GetPoints().GetPoint(0) #first sezione
            p1b = self.Sections_resampled[0].selectSurface().GetPoints().GetPoint(0) #first boundaries
            
            #controllo che la FrenetTangent sia circa parallelo alla normale della sezione
            ...
            # calcolo la proiez ortog sul piano ||P,n||
            vec1 = vtk_methods.proiezionePuntosuPiano(p0s, k1, p1b)
            #ora calcolo l'id del nuovo first
            id, point = self.Sections_resampled[self.selectSection('last')].findFirstAngle(vtk_methods.vettore(vec1, p0s))
            if debug == 1:
                ac=[]
                ac += vtk_methods.CreateVersor(p0s, i1,  colors[1])
                ac += vtk_methods.CreateVersor(p0s, j1, colors[2])
                ...
                ac.append(vtk_methods.CreateSphere(p1b, 0.31, colors[4]))
                ac.append(vtk_methods.CreateSphere(vec1, 0.31, colors[6]))          
                ac+=self.Sections_resampled[0].createActor()
                ac+=self.Sections_resampled[self.selectSection('first')].createActor()
                
                self.viewActors(ac)
            return id, vec1
            
    def findFirstProjection(self,pto=None, id=None,  out=None):        
        """ Calcola l'id del nuovo punto first delle sezioni andando a trovare il punto che ha angolo minore tra la proiez ortogonale del punto dato
        e il vettore(punto centrale sezione, punto intersezione piano sezione e retta parallela alla congiungente dei punti centrali sezione-boundaries passante per il punto 1 delle boundaries
        """
        debug =  0
        if out == 'in':
            ...
            p0s = self.Sections_resampled[id].clPoint  #punto centrale sezione            
            p1s = self.Sections_resampled[id].selectSurface().GetPoints().GetPoint(0) #first sezione
            p1b = pto
            
            i1 = vtk_methods.vettore(p0s, p1s)
            k1 = self.Sections_resampled[id].Normal
            j1= vtk_methods.prodottovettoriale(i1, k1) #n x v
            # calcolo la proiez ortog sul piano ||P,n||
            vec1 = vtk_methods.proiezionePuntosuPiano(p0s, k1, p1b)
            #ora cerco l'id del nuovo first
            id0 = 0            
            points = self.Sections_resampled[0].selectSurface().GetPoints()
            vec1versore = vtk_methods.normalize(vtk_methods.vettore(p0s, vec1))
            zero = abs(math.acos(vtk_methods.prodottoscalare(
                                                             vec1versore, 
                                                             vtk_methods.normalize(vtk_methods.vettore(p0s, points.GetPoint(0))))))
            for i in range(points.GetNumberOfPoints()):
                a = abs(math.acos(vtk_methods.prodottoscalare(
                                                              vec1versore, 
                                                              vtk_methods.normalize(vtk_methods.vettore(p0s, points.GetPoint(i))))))
                if a < zero:
                    id0 = i
                    zero = a
                    print i , a
#            id = self.Sections_resampled[self.selectSection('last')].findFirstAngle(vtk_methods.vettore(p0s, vec1))
            if debug == 1:
                ac=[]
                points = self.Sections_resampled[id].selectSurface().GetPoints()
                for i in range(0, points.GetNumberOfPoints()):
                    if i== 0:
                        ac.append(vtk_methods.CreateSphere(points.GetPoint(i), 0.1, [1, 0, 1]))
                    else:
            ...
            if debug == 1:
                ...
                ac.append(vtk_methods.CreateSphere(p1b, 0.31, colors[4]))
                ac.append(vtk_methods.CreateSphere(vec1, 0.31, colors[6]))          
                ac+=self.Sections_resampled[id].createActor()
                ac+=self.Sections_resampled[len(self.Sections_resampled)-1].createActor()
                
                self.viewActors(ac)
            return id0, points.GetPoint(id0)
            ...
            #ora cerco l'id del nuovo first
            id0 = 0            
            points = self.Sections_resampled[1].selectSurface().GetPoints()
            zero = abs(math.acos(vtk_methods.prodottoscalare(vtk_methods.normalize(vtk_methods.vettore(p0s, vec1)), vtk_methods.normalize(vtk_methods.vettore(p0s, points.GetPoint(0))))))
            for i in range(points.GetNumberOfPoints()):
                a = abs(math.acos(vtk_methods.prodottoscalare(vtk_methods.normalize(vtk_methods.vettore(p0s, vec1)), vtk_methods.normalize(vtk_methods.vettore(p0s, points.GetPoint(i))))))
                if a < zero:
                    id = i
                    zero = a
                    print i ,id0,  a
#            id = self.Sections_resampled[self.selectSection('last')].findFirstAngle(vtk_methods.vettore(vec1, p0s))
            if debug == 1:
                ac=[]
                ac += vtk_methods.CreateVersor(p0s, i1,  colors[1])
                ac += vtk_methods.CreateVersor(p0s, j1, colors[2])
                ...
                self.viewActors(ac)
            return id, points.GetPoint(id0)
            
    def selectSection(self, first=None):
        """ Seleziona la prima/ultima sezione non None che non sia una boundaries
        first: 'first/None' seleziona la prima, 'last' ultima
        """
        i=0
        if first == None or first=='first':
            i=0
            while i < len(self.Sections_resampled):
                if self.Sections_resampled[i] != None and self.Sections_resampled[i].Boundaries_flag == None:
                    return i
                else:
                    i+=1
        elif first == 'last':
            i = len(self.Sections_resampled)-1
            while i > 0 :
                try:
                    if self.Sections_resampled[i] != None and self.Sections_resampled[i].Boundaries_flag == None:
                        return i
                    ...
                    print 'BC: '+str(self.GroupId)+' i:'+str(i)
                    raise
        if i == len(self.Sections_resampled) or i == 0:
            errori.append('BC:'+str(self.GroupId)+' selectSection '+' type:'+first+'ERROR, didnt find a suitable section in the criteria')
            print 'BC:'+str(self.GroupId)+' selectSection '+' type:'+first+'ERROR, didnt find a suitable section in the criteria'
    ...
    def ReorderPoint(self, reorder = [None, None], out= None):
        ...
        reorder [0] == 
         ...
         - new: riordina tutte le sezioni con un nuovo first
        """
        if reorder[0] == None and reorder[1] == None:
            ...
            for i in range(len(self.Sections_resampled)-1):
                if self.Sections_resampled[i] != None and self.Sections_resampled[i].selectSurface().GetNumberOfPoints() > 0:
                    if self.Sections_resampled[i].Boundaries_flag == None:
                        try:
                            self.Sections_resampled[i].findFirstAngle()
            ...
            print 'in boundaries'
            if out == 'in':
                ids = self.findFirstBoundariesDistance(reorder[1], 1)
#                print ids
                for i in range(len(self.Sections_resampled)-1):
...
#                        print str(i)+'/'+ str(len(ids))+' '+str(ids[i])
                ...
                ids = self.findFirstBoundariesDistance(reorder[1], 0)
#                print ids
                for i in range(len(self.Sections_resampled)-1):
                    if self.Sections_resampled[i] != None and self.Sections_resampled[i].Boundaries_flag == None and self.Sections_resampled[i].selectSurface().GetNumberOfPoints() > 0:
#                        print i, len(ids)
            ...
            print 'In - new'
            if out == 'in':
                id, pto = self.findFirstBoundariesAngle(reorder[1], 'in')
#                for i in range(len(self.Sections_resampled)-1):
                i = self.selectSection('last')
                if self.Sections_resampled[i] != None:
                    self.Sections_resampled[i].reorderFromPointGiven(id)
            elif out == 'out1':
                id = self.findFirstBoundariesAngle(reorder[1], 'out')
                for i in range(len(self.Sections_resampled)-1):
                    if self.Sections_resampled[i] != None:
                        self.Sections_resampled[i].reorderFromPointGiven(id)
                        
    def ReorderPoint2(self, p, out = None):
        debug = 2
        if out == 'in':
            # Cerco l'id sulla penultima sezione prima della sezione relativa alle boundaries
            id, point = self.findFirstBoundariesAngle(None, 'in')
            k = self.selectSection('last')
            while k > 0:
                if self.Sections_resampled[k] != None and self.Sections_resampled[k].Boundaries_flag == None:
                    if debug ==1:print 'BC: '+str(self.GroupId)+'RIORDINO la sezione:', str(k)+'/'+str(self.selectSection('last')),' con punto', id
                    if k == self.selectSection('last'): 
                        if debug ==1:print 'last'
                        self.Sections_resampled[k].reorderFromPointGiven(id)                        
                    else:
                        id, point = self.Sections_resampled[k].reorderFromProjection(point)
                k+=-1
            print 'end IN'
                    
        elif out == 'out':
            id, point = self.findFirstBoundariesAngle(None, 'out')
            if debug == 2: 
            ...
            for k in range(len(self.Sections_resampled)):
                if self.Sections_resampled[k] != None and self.Sections_resampled[k].Boundaries_flag == None:
                    if debug ==2:print 'BC: '+str(self.GroupId)+' RIORDINO la sezione:', str(k)+'/'+str(self.selectSection('last')),' con punto', id
                    if k == self.selectSection('first'): 
                        if debug ==2:print 'first'
                        point = self.Sections_resampled[k].reorderFromPointGiven(id) 
...
class section(vessel): 
    def __init__(self, surf=None, fn=None,clPoint=None,  ID=None):
        #DATI
        self.Surface = surf        
        ...
        if fn != None:
            ...
            self.fn_resampled = self.fn#[0:len(self.fn)-4]+'_resampled.vtp'        
        #ELABORAZIONE
        if surf != None and fn != None:
            self.Origin_points = self.Surface.GetPoints()
            self.midPoint= self.centralPoint()     
            self.__meanRadius()
...
#                self.actor.append(vtk_methods.CreateSphere(self.Surface.GetPoints().GetPoint(i) , 0.1, [0, 1, 1]))
    ...
    def setCoords(self, fb = None, fn = None, ft = None, na = None, ab = None):
        ...
        u = vtk_methods.vettore(self.clPoint, self.selectSurface().GetPoints().GetPoint(0))
        v = vtk_methods.vettore(self.clPoint, self.selectSurface().GetPoints().GetPoint(1))
        w = vtk_methods.normalize(vtk_methods.prodottovettoriale(u, v))
        # controllo che un vettore normale alla sezione sia circa parallelo alla FrenetTangent
#        if math.acos(vtk_methods.prodottoscalare(w, self.FrenetTangentArray)) < (math.pi/180)*5:
#            self.Normal = self.FrenetTangentArray
#        else:
        self.Normal = w
    ...
    def selectSurface(self):
        ...
        if self.Surface_resampled != None:
            if self.Surface_resampled.GetNumberOfPoints()>0:
                if debug ==1:print self.ID, 'surface_resampled Selected!'
                return self.Surface_resampled
            else:
                if debug ==1:print self.ID, 'surface Selected!'
                return self.Surface
        else:
            if debug ==1:print self.ID, 'surface Selected!'
            return self.Surface
    
    def findFirstAngle(self, v = None):
        """ Cerca il punto della sezione con angolo minore rispetto al vettore v
        v == None -> ParallelTransportNormals
        """
        debug = 1
        if debug == 1:print '--- findFirstAngle --- section:'+str(self.ID), v
        surf = self.selectSurface()
        if v == None:
            
            zero = math.acos(vtk_methods.prodottoscalare(
                                          self.NormalsArray, 
                                          vtk_methods.normalize(vtk_methods.vettore(self.clPoint, surf.GetPoints().GetPoint(0)))
                                          ))
            first = 0
            for i in range(self.Surface_resampled.GetNumberOfPoints()):
                a = math.acos(vtk_methods.prodottoscalare(
                                           self.NormalsArray,
                                           vtk_methods.normalize(vtk_methods.vettore(self.clPoint, surf.GetPoints().GetPoint(i)))
                                           ))
                if a < zero:
                    first = i
                    zero = a
            self.first = first
            if debug == 1:print first, zero     
            return first, zero
        elif v != None:
            v = vtk_methods.normalize(v)
            print v, vtk_methods.normalize(vtk_methods.vettore(self.clPoint, surf.GetPoints().GetPoint(0)))
            zero = abs(math.acos(vtk_methods.prodottoscalare(
                                      v, 
                                      vtk_methods.normalize(vtk_methods.vettore(self.clPoint, surf.GetPoints().GetPoint(0)))
                                      )))
            ...
            for i in range(surf.GetNumberOfPoints()):
                ...
                if debug == 1:print 'Section:', self.ID, i, first, a
                if a < zero:                    
                    first = i
                    zero = a                    
            self.first = first    
    ...
    def printPoints(self, dir, k):
        ...
        out_file.close()
    def reorderPoints(self, pto=None):
        """ Riordina i punti congruentemente con il punto first
        """
        debug = 0
        
        if pto != None:
            for i in range(self.selectSurface().GetNumberOfPoints()):
                if self.selectSurface().GetPoints().GetPoint(i) == pto:
                    self.first = i                    
        ...
        if self.Boundaries_flag == None:
            ...
            if debug == 1:
                ...
                print 'Surf: '+str(self.ID), surf!= None, 'id first:', self.first
            ...
            self.Surface_resampled.SetPoints(sectionPoints)
            self.Surface_resampled.SetPolys(sectionCellArray)
#            print 'pre', surf.GetPoints().GetPoint(1), self.Surface_resampled.GetPoints().GetPoint(1)
            self.Update( self.Surface_resampled)
#            print 'post', self.Surface_resampled.GetPoints().GetPoint(1)
            
        else:
            print 'WARNING: boundaries_flag:', self.Boundaries_flag
                
        
    def reorderFromPointGiven(self,id=None):
        """ Riordina i punti a partire dal punto passato come parametro, si appoggia al metodo reorderPoints, modificando self.first
        """
        if id!= None:
            print 'in reorderFromPointGiven: first',self.first, 'id:', id 
            self.first = id
            self.reorderPoints()
            return  self.Surface_resampled.GetPoints().GetPoint(0)
            
    def reorderFromProjection(self, pto=None, debug = 0):
        debug = 0
        p0s = self.clPoint  #punto centrale sezione            
        p1s = self.selectSurface().GetPoints().GetPoint(0) #first sezione
        p1b = pto
        ...
        #ora cerco l'id del nuovo first
        id = 0            
        points = self.selectSurface().GetPoints()
        vec1versore = vtk_methods.normalize(vtk_methods.vettore(p0s, vec1))
        zero = abs(math.acos(vtk_methods.prodottoscalare(
                                                         vec1versore, 
                                                         vtk_methods.normalize(vtk_methods.vettore(p0s, points.GetPoint(0))))))
        ...
        if debug == 1:
                ...
                for i in range(0, points.GetNumberOfPoints()):
                    if i== 0:
                        ...
                        ac.append(vtk_methods.CreateSphere(points.GetPoint(i), 0.1, [1, 0, 0]))
                    else:
                        ac.append(vtk_methods.CreateSphere(points.GetPoint(i), 0.1, [0, 0, 1]))
#                    print i, math.acos(vtk_methods.prodottoscalare(vtk_methods.normalize(vtk_methods.vettore(p0s, vec1)), vtk_methods.normalize(vtk_methods.vettore(p0s, points.GetPoint(i)))))
                    
                ac += vtk_methods.CreateVersor(p0s, vtk_methods.vettore(p0s, vec1), colors[6])
                ac.append(vtk_methods.CreateSphere(vec1, 0.11, colors[6]))
                self.viewActors(ac)
        self.reorderFromPointGiven(id)
        return id, points.GetPoint(id)
    
        
    def Reader(self, file=None):
        b = os.path.splitext(file)
        print b
        if file != None:
            if b[1]=='.stl':
                ...
                points = vtk_methods.ReadTxt(file)
                self.createSectionFromPoints(points)

                print 'File: ', file, ' read!'
                self.fn = b[0]+'.stl'
            else:
                errori.append('(section,reader): Estensione non supportata')
                return None
            if self.Surface == None:
                errori.append('(section.reader): La superficie non e stata letta corretamente')
                return None
        elif surf != None:
            self.Surface = surf
        else:
            errori.append('(section.reader): parametri mancanti')            
            return None                
        # Scrivo il file
        self.fn = vtk_methods.WritePolyData(self.Surface, self.fn)
    def createSectionFromPoints(self, points=None):
        """ Crea una sezione a partire dai punti
        """
        midPoint=[0, 0, 0]
        Lines = vtk.vtkCellArray()
        ...
        for i in range(len(points)):
            Lines.InsertCellPoint(i)
            Points.InsertNextPoint(points[i])
            for k in range(3):
                midPoint[k] += points[i][k]
        for k in range(3):
            midPoint[k] = midPoint[k]/len(points)
        Lines.InsertCellPoint(0)
        Polygon = vtk.vtkPolyData()
        Polygon.SetPoints(Points)
        Polygon.SetLines(Lines)
